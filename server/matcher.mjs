...
export function normalizeProfile(extracted, rawText, data) {
  ...
  const targetKeywords = [...new Set((targetGroup ? targetGroup.keywords : freeKeywords).map((word) => word.toLocaleLowerCase("zh-CN")))].slice(0, 18);
  const major = resolveKnownValue(extracted && extracted.major, filters.majors || []);
...
export function getAiCandidates(data, profile) {
  let cases = (data.cases || []).filter((item) => {
    ...
    const searchable = (String(app.program) + " " + String(item.major || "")).toLocaleLowerCase("zh-CN");
    if (profile.targetKeywords.length && !profile.targetKeywords.some((word) => searchable.includes(word))) return false;
  ...
  return [...grouped.values()].map((entry) => {
    ...
    return { item: entry.item, historicalAverage, rank, delta: historicalAverage + rankBonus - applicantAverage, count: entry.count, caseIds: entry.caseIds, samples: entry.samples };
  });
}

export function chooseAiTiers(candidates) {
  const used = new Set();
  const pick = (predicate, anchor) => {
    ...
    const chosen = preferred
      .sort((a, b) => Math.abs(a.delta - anchor) - Math.abs(b.delta - anchor) || (a.rank === null ? 9999 : a.rank) - (b.rank === null ? 9999 : b.rank))
      .slice(0, 6);
